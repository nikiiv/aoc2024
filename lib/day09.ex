defmodule Day09 do
  ...
  def swap(list, start_index, end_index, to_index) do
    ...
    arr = [Enum.slice(list, 0..(to_index - 1)),
      Enum.slice(list, start_index..end_index),
  ...
  def solve_2 do
    ...
    res |> Enum.with_index() |> Enum.filter(fn {c,_}-> c != "." end) |> Enum.map(fn {a,b} -> a*b end) |> Enum.sum
  end
  ...
  def handle_file_id(file_id, list) do
    ...
    new_list = if to_index==nil || to_index > start_index, do: list, else: swap(list, start_index, end_index, to_index-1)
