defmodule Memento do
  ...
  def new, do: %{}
  def get(memento, num, step), do: Map.fetch(memento, {num, step})
  def put(memento, num, step, val), do: Map.put(memento, {num, step}, val)
end

defmodule Day11a do
  @data [1117, 0, 8, 21078, 2_389_032, 142_881, 93, 385]
  # data [1234123]

  def solve(memento, _single_num, 0), do: {1, memento}

  def solve(memento, single_num, step) do
    case Memento.get(memento, single_num, step) do
      {:error} -> compute(memento, single_num, step)
      {:ok, val} -> {val, memento}
    end
  end
  ...
  def compute(memento, single_num, step) do
    ...
    {steps, new_memento} =
      cond do
        ...
        true ->
          solve(memento, single_num * 2024, step - 1)
      end
